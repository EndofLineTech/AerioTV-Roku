sub main()
    ...
    guideCachePrune(cache, 710)
    assertEqual(cache.entries.doesExist("0"), false, "stale display grace expires at its boundary")
    assertEqual(cache.entries.count(), 2, "newer coverage is retained")
    guideCachePrune(cache, 5)
    assertEqual(cache.entries.count(), 0, "clock rollback invalidates future-fetched memory entries")
    assertEqual(cache.order.count(), 0, "coverage order pruned with entries")
